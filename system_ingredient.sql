/*
 SCRIPT TO CREATE THE TABLE SYSTEM INGREDIENT
 */
CREATE TABLE `tbl_system_ingredient` (
    `clm_id` INT NOT NULL AUTO_INCREMENT,
    `clm_name` TEXT(30) NOT NULL,
    `clm_description` TEXT(300) NOT NULL,
    `clm_amount` DOUBLE NOT NULL DEFAULT 0,
    `clm_stock_control` INT(1) NOT NULL DEFAULT '0',
    `clm_is_active` INT(1) NOT NULL DEFAULT '1',
    `clm_company_rel` INT NOT NULL,
    `clm_ingredient_category_rel` INT NOT NULL,
    `clm_unit_measurement_rel` INT NOT NULL,
    ...
    FOREIGN KEY (clm_ingredient_category_rel)  REFERENCES tbl_system_ingrediente_category(clm_id),
    FOREIGN KEY (clm_unit_measurement_rel)  REFERENCES tbl_system_unit_measurement(clm_id)
);